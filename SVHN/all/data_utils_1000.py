...
class DataSet(object):
  ...
  def __init__(self, hparams):
    ...
    assert hparams.train_size + hparams.validation_size <= 604388

    # Determine how many images we have loaded
    train_dataset_size = 604388
    test_dataset_size = 26032
    all_data = np.empty((train_dataset_size, 32, 32, 3), dtype=np.uint8)
    all_labels = np.empty((train_dataset_size, ), dtype=np.uint8)
    test_data = np.empty((test_dataset_size, 32, 32, 3), dtype=np.uint8)
    tf.logging.info('SVHN')
    datafile = 'train_32x32.mat'
    extra_datafile = 'extra_32x32.mat'
    num_classes = 10

    d = load(os.path.join(hparams.data_path, datafile))
    d_extra = load(os.path.join(hparams.data_path, extra_datafile))
    all_data[0:73257] = copy.deepcopy(d['X'])
    all_data[73257:] = copy.deepcopy(d_extra['X'].transpose((3, 0, 1, 2)))
    label = d['y'][0]
    extra_label = d_extra['y'][0]
    all_labels[0:73257] = copy.deepcopy(label)
    all_labels[73257:] = copy.deepcopy(extra_label)

    d_test = load(os.path.join(hparams.data_path, 'test_32x32.mat'))
    ...
    assert 604388 >= train_size + val_size
