...
th = handlers.TimedRotatingFileHandler(filename="../CIFAR/select/result", encoding='utf-8')
...
class BayesianOptimization(BaseSolver):
    ...
    def run(self, num_iterations=100, X=None, y=None, p=0):
        """
        ...
        for it in range(self.init_points, num_iterations):
            ...
            # Evaluate
            start_time = time.time()
            new_y = self.objective_func(new_x, it)
            self.time_func_evals.append(time.time() - start_time)
            ...
            if it % 10 == 0:
                tt = len(self.incumbents_values)
                self.painter(tt, self.incumbents_values, it//10)
        ...
        tt = len(self.incumbents_values)
        self.painter(tt, self.incumbents_values, p)
        print("incumbents_values:%s" %self.incumbents_values)
        #self.write_to_file(self.incumbents_values)
    ...
    def painter(self, tt, y=None, i=0):
        
        T = [i for i in range(tt)]
        #y_real = [-3.322368011391339 for i in range(tt)]
        plt.figure()
        #plt.plot(T, y_real)
        plt.plot(T, y)
        plt.savefig('../CIFAR/select/result/%s.png' %i)