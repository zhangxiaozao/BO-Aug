...
import CIFAR.select.custom_ops as ops
from CIFAR.select import data_utils_4000
from CIFAR.select import helper_utils
import numpy as np
from CIFAR.select.shake_drop import build_shake_drop_model
from CIFAR.select.shake_shake import build_shake_shake_model
import tensorflow as tf
from CIFAR.select.wrn import build_wrn_model
from CIFAR.select import bo_policies
...
class CifarModelTrainer(object):
  ...
  def __init__(self, hparams, temp_checkpoint_dir):
    self._session = None
    self.hparams = hparams

    self.model_dir = os.path.join(temp_checkpoint_dir, 'model')
    self.log_dir = os.path.join(temp_checkpoint_dir, 'log')
...
def main(policies, time):
  ...
  if MODEL_NAME == 'wrn':
  ...
  elif MODEL_NAME == 'shake_shake_32':
  ...
  elif MODEL_NAME == 'shake_shake_96':
    hparams.add_hparam('model_name', 'shake_shake')
    hparams.add_hparam('num_epochs', 1800)
    hparams.add_hparam('shake_shake_widen_factor', 6)
    hparams.add_hparam('lr', 0.01)
    hparams.add_hparam('weight_decay_rate', 0.001)
  elif MODEL_NAME == 'shake_shake_112':
    hparams.add_hparam('model_name', 'shake_shake')
    hparams.add_hparam('num_epochs', 1800)
    ...
    raise ValueError('Not Valid Model Name: %s' % MODEL_NAME)
  bo_policies.construct_good_policies(policies)
  temp_checkpoint_dir = CHECKPOINT_DIR + str(time)
  cifar_trainer = CifarModelTrainer(hparams, temp_checkpoint_dir)
  acc = cifar_trainer.run_model()
  bo_policies.delete_exp0s()
  return acc